...
\begin{eqnarray}
E (&R_1&, R_2, \ldots, R_N; \mathbf{c}) = \sum_{k=1}^{M_1} \sum_{p=1}^{N} \frac{c_{pk}}{R_p^k} + \sum_{k,m=1}^{M_2} \sum_{p>q}^{N} \frac{c_{pkqm}}{R_p^k R_q^m} + \nonumber \\
...
Ordinary least squares (OLS) method finds the best parameters in the linear regression model~(\ref{eq:energy}) by minimizing objective function with respect to $\mathbf{c}$
%
\begin{eqnarray}
\Delta_{\text{OLS}} = \frac{1}{2} \sum_{i=1}^{T} \left( E_i -  E (\mathbf{R}_{i}; \mathbf{c}) \right)^2 \label{eq:OF-OLS}
\end{eqnarray}
%
where $\mathbf{R}_i$ STOP.
