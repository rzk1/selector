...
Therefore, we designed a procedure that replaces, adds, and eliminates collinear features while keeping the model function both compact and accurate. [VLAD: does the fine-tuning algorithm add features?]
...
Next, each feature is assigned a list of strongly-correlated features. Two features are considered strongly correlated if their correlation coefficient is greater than a pre-selected threshold value of approximately 0.8 [VLAD: what are typical MinCorrelation values?].
...
%VLAD: place figures 1,2,3 into one Figure with 3 panels A, B, C. Place figures 5,6,7 into one figure with 3 panels A, B, C. Combine figures 4 and 8 into one panel.
% B2
\begin{figure}
